...
function LogDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-white pb-32 animate-in fade-in slide-in-from-bottom-8 duration-700">
      {/* Sticky Header */}
      <div className="sticky top-0 z-50 ios-blur bg-white/80 px-6 pt-12 pb-6 flex items-center justify-between border-b border-zinc-50 shadow-sm shadow-zinc-50/50">
        <button
          onClick={() => window.history.back()}
          className="w-12 h-12 bg-zinc-50 rounded-3xl flex items-center justify-center text-zinc-900 border border-zinc-100/50 tap-effect"
        >
        ...
        <button
          ...
          disabled={isDeleting}
          className="w-12 h-12 bg-rose-50 rounded-3xl flex items-center justify-center text-rose-500 border border-rose-100/50 tap-effect disabled:opacity-50"
        >
      ...
      <div className="px-6 space-y-8 mt-4">
        <div className="relative aspect-square w-full bg-zinc-50 rounded-[56px] overflow-hidden border border-zinc-100 shadow-2xl shadow-zinc-100/50 group">
          {imageUrl ? (
            <img
              src={imageUrl}
              alt={log.food_name}
      ...
      <div className="px-8 mt-10 space-y-10">
        <div className="space-y-4">
          <h1 className="text-5xl font-black tracking-tighter text-zinc-900 leading-[0.9] italic">
            {log.food_name}
          </h1>
        ...
        {/* Big KCAL Card */}
        <div className="bg-zinc-900 rounded-[48px] p-10 text-white relative overflow-hidden shadow-2xl shadow-purple-200/20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          {[
            ...
            <div
              key={macro.label}
              className="bg-zinc-50 p-6 rounded-[40px] border border-zinc-100 flex sm:flex-col items-center sm:items-start justify-between sm:justify-start gap-4 sm:space-y-4"
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 bg-white rounded-3xl flex items-center justify-center ${macro.iconColor} shadow-sm shrink-0`}
                >
              ...
              <div className="flex-1 max-w-25 sm:max-w-none sm:w-full h-1.5 bg-zinc-200 rounded-full overflow-hidden">
                <div
